    ...
    public function up(): void
    {
        if (! Schema::hasTable('panels')) {
            Schema::create('panels', function (Blueprint $table) {
                ...
                $table->string('name');
                $table->string('url');
                $table->enum('panel_type', ['marzban', 'marzneshin', 'xui', 'v2ray', 'eylandoo', 'other'])->default('marzban');
        ...
        if (Schema::hasTable('panels')) {
            ...
            Schema::table('panels', function (Blueprint $table) use ($columns) {
                if (! in_array('panel_type', $columns)) {
                    $table->enum('panel_type', ['marzban', 'marzneshin', 'xui', 'v2ray', 'eylandoo', 'other'])->default('marzban')->after('url');
                }
