...
class ConfigsRelationManager extends RelationManager
{
        ...
        return $table
            ...
            ->actions([
                ...
                Tables\Actions\Action::make('copy_url')
                    ...
                    ->color('info')
                    ->visible(fn (ResellerConfig $record): bool => !$record->isOvpanel())
                    ->action(function (ResellerConfig $record) {
                        Notification::make()
                            ->success()
                            ->title('لینک کپی شد')
                ...
                Tables\Actions\Action::make('download_ovpn')
                    ->label('دانلود .ovpn')
                    ->icon('heroicon-o-arrow-down-tray')
                    ->color('success')
                    ->visible(fn (ResellerConfig $record): bool => $record->isOvpanel())
                    ->modalHeading('دانلود فایل .ovpn')
                    ->modalDescription(fn (ResellerConfig $record): string => "فایل .ovpn برای کاربر: {$record->external_username}")
                    ->modalContent(function (ResellerConfig $record) {
                        $downloadUrl = route('ovpn.download.token', ['token' => $record->ovpn_token]);
                        
                        return view('filament.pages.ovpn-download-modal', [
                            'downloadUrl' => $downloadUrl,
                            'username' => $record->external_username,
                        ]);
                    })
                    ->modalSubmitAction(false)
                    ->modalCancelActionLabel('بستن'),

                Tables\Actions\Action::make('refresh_ovpn')
                    ->label('بروزرسانی .ovpn')
                    ->icon('heroicon-o-arrow-path')
                    ->color('warning')
                    ->visible(fn (ResellerConfig $record): bool => $record->isOvpanel())
                    ->requiresConfirmation()
                    ->action(function (ResellerConfig $record) {
                        $this->refreshOvpnFile($record);
                    }),

                Tables\Actions\Action::make('copy_download_link')
    ...
    protected function refreshOvpnFile(ResellerConfig $config): void
    {
        try {
            if (!$config->isOvpanel()) {
                throw new \Exception('Not an OV-Panel config');
            }
            ...
            $result = $service->refreshOvpn($config->panel_user_id);

            if (!$result['success']) {
                throw new \Exception($result['error'] ?? 'Failed to refresh .ovpn file');
            }

            // Store the new .ovpn file
            $filename = \Illuminate\Support\Str::uuid() . '.ovpn';
            $path = "ovpn/{$filename}";
            \Illuminate\Support\Facades\Storage::put($path, $result['ovpn_content']);

            // Delete old file if exists
            if ($config->ovpn_path && \Illuminate\Support\Facades\Storage::exists($config->ovpn_path)) {
                \Illuminate\Support\Facades\Storage::delete($config->ovpn_path);
            }
            ...
            Notification::make()
                ->success()
                ->title('فایل .ovpn با موفقیت بروزرسانی شد')
                ->body('توکن دانلود نیز تغییر کرد')
                ->send();
        } catch (\Exception $e) {
            Log::error("Error refreshing .ovpn file: " . $e->getMessage(), ['config_id' => $config->id]);
            Notification::make()
                ->danger()
                ->title('خطا در بروزرسانی فایل .ovpn')
                ->body($e->getMessage())
                ->send();
        }
    }
