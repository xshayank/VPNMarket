...
## Overview
The reseller feature allows bulk VPN config provisioning and management with automatic traffic enforcement, grace thresholds, and comprehensive audit logging.

## Key Features

### 1. Traffic-Based Reseller Accounts
- Aggregate traffic quota across multiple configs
- Per-reseller time windows
- Automatic suspension when quota exhausted or window expired
...
- **Grace Thresholds**: Configurable percentage and absolute grace to prevent premature disables
...
- `window_starts_at`: Start of current traffic window
- `window_ends_at`: End of current traffic window
...
- `expires_at`: Config expiration time
...
2. For each reseller:
   ...
   - If `allow_config_overrun` is false, check per-config limits
   ...
   - If exceeded or window expired:
     ...
     - Auto-disable all active configs
...
**Process**:
1. Find suspended resellers with traffic remaining and valid window
...
- `reseller.auto_disable_grace_percent`: Grace percentage (0-10%)
- `reseller.auto_disable_grace_bytes`: Minimum grace in bytes
- `reseller.time_expiry_grace_minutes`: Grace time after expiration
