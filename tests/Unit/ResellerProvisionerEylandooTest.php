...
test('provisionEylandoo handles subResponse.sub_url shape from production', function () {
    Http::fake([
        '*/api/v1/users' => Http::response([
            'success' => true,
            'created_users' => ['test_user'],
    ...
    expect($result)->toBeArray()
        ->and($result['username'])->toBe('test_user')
        ->and($result['subscription_url'])->toBe('https://example.com/sub/resell_8_cfg_53');
});

test('provisionEylandoo handles subResponse.sub_url with relative path', function () {
    Http::fake([
        '*/api/v1/users' => Http::response([
            'success' => true,
            'created_users' => ['test_user'],
    ...
    $panel = Panel::factory()->eylandoo()->create([
        ...
        'extra' => [
            'node_hostname' => 'https://panel.example.com',
        ],
    ]);
    ...
    expect($result)->toBeArray()
        ->and($result['username'])->toBe('test_user')
        ->and($result['subscription_url'])->toBe('https://panel.example.com/sub/resell_8_cfg_53');
});
