...
test('reseller provisioner stores clean absolute URL for Marzneshin', function () {
    Http::fake([
        '*/api/admins/token' => Http::response(['access_token' => 'test-token'], 200),
        '*/api/users' => Http::response([
            'username' => 'testuser',
    ...
    expect($result)->not->toBeNull()
        ->and($result['subscription_url'])->toBeString()
        ->and($result['subscription_url'])->toBe('https://node.example.com/sub/abc123')
        ->and($result['subscription_url'])->not->toContain('لینک سابسکریپشن')
        ->and($result['subscription_url'])->not->toContain("\n");
});

test('reseller provisioner stores clean absolute URL for Marzban', function () {
    Http::fake([
